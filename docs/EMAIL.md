...
| `smtp_host` | 可选，SMTP 主机；IMAP 主机为 `imap.exmail.qq.com` 且留空时自动使用 `smtp.exmail.qq.com`，其他服务商需填写 |
| `smtp_port` | 默认 `465` |
...
网页登录后进入“邮箱配置”，可新增、编辑、删除最多 100 个邮箱。配置接口仅接受中央 OAuth 会话，读写 API Key 均不能管理邮箱；跨来源写入会被拒绝。列表和编辑接口不返回密码，只显示是否已设置，并展示每个邮箱的收信状态；编辑时密码留空表示沿用原值，更换密码会清除该邮箱的失败退避。删除配置不会删除已入库邮件或历史游标。
...
`mailbox_auth_failed` 表示邮箱服务商拒绝 IMAP 登录。若刚开启 IMAP，先确认企业管理员允许客户端访问，再核对邮箱是否启用安全登录及是否需要客户端专用密码；在页面重新填写密码后可立即测试，后台轮询也会清除原失败退避。不要在聊天、日志或工单中粘贴密码。
